...
describe('light-first theme styles', () => {
  ...
  test('uses one combined transformation page and a compact dark contact band', () => {
    ...
    expect(styles).toMatch(/\.beside-work__item\s*\{[^}]*display:\s*flex;[^}]*flex-direction:\s*column;[^}]*border:\s*0;[^}]*background:\s*transparent;[^}]*transform:\s*translateY\(0\);[^}]*transition:\s*transform 600ms cubic-bezier\(\.6, 0, \.25, 1\)/s);
    expect(styles).toMatch(/\.beside-work__item > img,[\s\S]*?\.beside-work__photo-collage\s*\{[^}]*aspect-ratio:\s*3\s*\/\s*2;[^}]*clip-path:\s*polygon\(0 0, 100% 0, 100% 100%, 0 100%\);[^}]*transition:\s*clip-path 600ms cubic-bezier\(\.6, 0, \.25, 1\)/s);
    ...
    expect(styles).toMatch(/\.beside-work__gallery-prompt\s*\{[^}]*display:\s*block;[^}]*border:\s*0;[^}]*background:\s*transparent;[^}]*color:\s*var\(--accent-text\);[^}]*text-decoration:\s*underline/s);
    expect(styles).toMatch(/img\[src\$='world-low-pixels\.svg'\]\s*\{[^}]*background:\s*#000;[^}]*object-fit:\s*contain/s);
    expect(styles).toMatch(/\.beside-work__item::before\s*\{[^}]*inset:\s*-8px;[^}]*border:\s*1px solid[^}]*background:\s*color-mix\(in srgb, var\(--accent\) 5%, var\(--surface\)\);[^}]*box-shadow:\s*0 24px 56px[^}]*opacity:\s*0;[^}]*transition:\s*opacity 600ms cubic-bezier\(\.6, 0, \.25, 1\)/s);
    expect(styles).toMatch(/\[data-theme='dark'\] \.beside-work__item::before\s*\{[^}]*background:\s*color-mix\(in srgb, var\(--accent\) 8%, var\(--surface-alt\)\);[^}]*box-shadow:\s*0 28px 60px/s);
    expect(styles).toContain('.beside-work__item:hover { transform: translateY(-4px); }');
    expect(styles).toContain('.beside-work__item:focus-within { transform: translateY(-4px); }');
    expect(styles).toContain('.beside-work__item:hover > img');
    expect(styles).toContain('.beside-work__item:hover .beside-work__photo-collage');
    expect(styles).toContain('clip-path: polygon(16px 16px, calc(100% - 16px) 16px, calc(100% - 16px) 100%, 16px 100%);');
    expect(styles).toContain('.beside-work__item:focus-within .beside-work__item-content');
    expect(styles).toMatch(/@media \(max-width: 767px\)[\s\S]*?\.beside-work__item-content\s*\{[^}]*padding-top:\s*24px/s);
    expect(styles).toMatch(/@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.beside-work__item,[\s\S]*?transition:\s*none/s);
    expect(styles).toContain('.beside-work__item:is(:hover, :focus-within) { transform: none; }');
  ...
  test('floats and rounds both showcase cards while the media fills its frame', () => {
    ...
    expect(styles).toMatch(/@media \(max-width: 1100px\)[\s\S]*?\.strategic-showcase__copy--exit\s*\{[^}]*display:\s*none/s);
    expect(styles).not.toContain(".strategic-showcase__copy-shell:has(.strategic-showcase__copy[data-project-index='3'])");
  });
